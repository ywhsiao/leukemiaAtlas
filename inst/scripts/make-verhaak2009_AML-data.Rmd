---
title: Processing the verhaak2009_AML dataset
author: Yi-Wen Hsiao
...
We obtain a bulk RNA sequencing dataset of the AML subtypes from @verhaak2009prediction.
Expression matrix for endogenous genes are available from the Gene Expression Omnibus
using the accession number [GSE6891](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE6891).
...
```{r}
metadata <- pData(phenoData(gse[[1]]))[,c('title', 'risk:ch1', 'score:ch1')]
metadata$title <- 'AML'
metadata$`risk:ch1` <- gsub('cytogenetic ', '',metadata$`risk:ch1`)
metadata$`risk:ch1`[is.na(metadata$`risk:ch1`)] <- 'unknown'
unique(metadata$`score:ch1`)
metadata$`score:ch1` <-gsub('FAB ', '',metadata$`score:ch1`)
metadata$`score:ch1`[is.na(metadata$`score:ch1`)] <- 'unknown'
metadata$`score:ch1`[metadata$`score:ch1` == 'Mx'] <- 'unknown'
metadata <- metadata[!(metadata$`score:ch1`%in%c('RAEB-t', 'RAEB')),]
names(metadata) <- c('subtype', 'AML_risk', 'AML_subtype')
...
metadata <- metadata[,c('study_id','sample_id', 'subtype', 'platform', 'AML_subtype', 'AML_risk')]
...
```{r}
path <- file.path("leukemiaAtlas", "verhaak2009_AML")
