---
title: Processing the ceroi2016_acute dataset
author: Yi-Wen Hsiao
...
We obtain a bulk RNA sequencing dataset of the CML subtypes from @ceroi2016lxr.
Expression matrix for endogenous genes are available from the Gene Expression Omnibus
using the accession number [GSE89565](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE89565).
...
metadata$title <- ifelse(grepl('AML', metadata$title), 'AML', ifelse(grepl('ALL', metadata$title), 'ALL', NA))
metadata <- metadata[!is.na(metadata$title),]
...
```{r}
path <- file.path("leukemiaAtlas", "ceroi2016_acute")
