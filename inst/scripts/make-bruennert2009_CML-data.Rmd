---
title: Processing the bruennert2009_CML dataset
author: Yi-Wen Hsiao
...
We obtain a bulk RNA sequencing dataset of the CML subtypes from @bruennert2009early.
Expression matrix for endogenous genes are available from the Gene Expression Omnibus
using the accession number [GSE12211](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE12211).
...
metadata <- metadata[str_detect(metadata$title, 'CML'),]
metadata$title <- 'CML'
...
gpl <- getGEO('GPL571', destdir=tempdir())
colnames(Table(gpl))
...
path <- file.path("leukemiaAtlas", "bruennert2009_CML")
