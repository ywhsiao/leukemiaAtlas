---
title: Processing the chuang2015_AML dataset
author: Yi-Wen Hsiao
...
We obtain a bulk RNA sequencing dataset of the AML subtypes from @chuang2015mrna.
Expression matrix for endogenous genes are available from the Gene Expression Omnibus
using the accession number [GSE71014](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE71014).
...
metadata$platform <- 'Illumina HumanHT-12 V4.0'
head(metadata)
```
...
gpl <- getGEO('GPL10558', destdir=tempdir())
colnames(Table(gpl))
annot <- Table(gpl)[,c("ID","ILMN_Gene")]
...
path <- file.path("leukemiaAtlas", "chuang2015_AML")
